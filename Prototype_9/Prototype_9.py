...
def encode_data_quaternary(data_bytes, output_img_path, cell_size=5):
    ...
    padded_values = pixel_values + ([0] * padding_needed) # Pad with black

    # 5. Create the base image grid
    grid = np.array(padded_values, dtype=np.uint8).reshape((size, size))
    
    # 6. Create final image and upscale it for clarity
    img = Image.fromarray(grid, mode='L') # 'L' mode for grayscale
    
    # Use NEAREST filter to create a sharp, blocky look
    ...
    return size # Return the base grid size for the decoder
...
def decode_data_quaternary(input_img_path, original_data_len, grid_size):
    ...
    # 1. Load the image and downscale it to the original data grid size
...
if __name__ == "__main__":
    ...
    image_file = "encoded_quaternary_hq.png" # HQ for High Quality
    output_text_file = "recovered_quaternary.txt"
    CELL_SIZE_FOR_ENCODING = 5 # Increase this for a larger, clearer image
    ...
    if not os.path.exists(docx_file):
        ...
        if document_text:
            ...
            # Encoder now returns the base grid size
            ...
            #  2. DECODE 
            # Pass the grid size to the decoder
