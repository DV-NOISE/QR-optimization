...
def encode_sparse_compressed(data: bytes, size=160, cell_size=5):
    ...
    Args:
        ...
        size (int): Grid size
        cell_size (int): Pixel size for visualization
    ...
    img = Image.fromarray(grid).resize(
        (size*cell_size, size*cell_size), Image.NEAREST)
    return img
...
if __name__ == "__main__":
    ...
    encoded_img.save("sparse_160x160_compressed.png")
    print("Encoded image saved as 'sparse_160x160_compressed.png'")

    # Decode
    decoded_data = decode_sparse_compressed(encoded_img, size=160)
    print("Decoded Data:", decoded_data)
    print("Success:", decoded_data == data)
